...
def configure_utf8_stdio() -> None:
    """Make dispatcher text I/O independent of the Windows console code page."""
...
def ensure_supported_runtime() -> None:
    ...
    if platform.system() != 'Windows':
        raise RuntimeError('This edition requires a native Windows desktop and native Python.')
        ...
        if not help_only and not is_initialization_command(args.component, args.arguments):
            ...
            if not init.get("initialized"):
                raise RuntimeError(
                    ...
                    "obtain the user choice to continue, explicitly explain that a new Mem must use an xxx and a yyy that are each unique among WebMind Mem names on this computer, then use mem scan/name-info/init to select an external xxx-yyy-mem folder."
                )
