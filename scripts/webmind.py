...
from runtime import (ROOT, configure_utf8_stdio, data_dir, venv_python, MINIMUM_PYTHON,
                     require_native_host, is_wsl, windows_desktop, gui_preflight)
...
def prepare_input(component: str, arguments: list[str]) -> tuple[list[str], bytes | None]:
    ...
    # PowerShell-authored files commonly use CRLF. Normalize all supported text
    # files to the package-wide LF contract before forwarding bytes to a child CLI.
...
def doctor() -> int:
    ...
    result = {'ok': runtime_ok, 'runtime_ready': runtime_ok, 'gui_permissions_ready': gui_ready,
              ...
              'notes': ['No screenshot, mouse action or browser launch is performed by doctor.',
                        'runtime_ready does not prove that a real GUI task works. Check gui_permissions_ready and follow the low-risk checks in 使用教程.md.',
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(description='webmind-codex on Windows. Text commands accept --input-file FILE.')
        ...
        if not help_only and not is_initialization_command(args.component, args.arguments):
            ...
            if not init.get('initialized'):
                raise RuntimeError(
                    ...
                    'obtain the user choice to continue, explain that both xxx and yyy must each be unique among WebMind Mem folders on this computer, then use mem scan/name-info/init to select an external xxx-yyy-mem folder.'
                )
