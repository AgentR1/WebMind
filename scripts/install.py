#!/usr/bin/env python3
"""Install a self-contained Codex skill on native macOS."""
...
def parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description='Install webmind-codex on native macOS. Existing browser profiles and Mem are preserved.')
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        result.update({'ok': True, 'data_dir': str(data), 'dependencies_installed': not args.skip_deps,
                       'next': 'Restart Codex if needed, invoke $webmind-codex, and run doctor --json. macOS GUI permissions and Codex command approvals may still be required.'})
