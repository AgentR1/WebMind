#!/usr/bin/env python3
"""Manage an external Mem folder for Claude Code WebUse task memory."""
...
def global_template() -> str:
    return (
        "# Global memory\n\n"
        "Record only durable instructions and reusable cross-task memory that should apply to many Claude Code WebUse tasks.\n\n"
...
def rebuild_content(mem_path: Path) -> str:
    ...
    lines: List[str] = [
        "# Mem content index",
        "",
        "This file is maintained by `webuse-mem`. It records the current architecture of this `Mem` folder so Claude Code can choose only relevant memories.",
        "",
        "## Root",
        "",
        "- path: `Mem/`",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Manage a Claude Code WebUse Mem folder.")
