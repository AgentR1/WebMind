---
name: webuse-mem
description: Claude Code skill for selective durable memory in concrete WebUse, browser, Chrome, GUI automation, form-filling, web-reading, Gmail, Zhihu, and screen-based tasks. At task start, load only relevant memory from the selected Mem folder. At task end, reconcile new evidence with old guidance by adding, editing, reordering, replacing, or removing experience so the result remains canonical and internally consistent.
---

# webuse-mem

Use this skill when Claude Code needs selective, durable memory for WebUse-style tasks. The memory lives outside the skill in a folder named `Mem`. Treat memory as a maintained guide, not an append-only activity log.
...
```powershell
& "$env:CLAUDE_PLUGIN_ROOT\scripts\webuse.ps1" mem <command> [options]
```

```bash
bash "$CLAUDE_PLUGIN_ROOT/scripts/webuse.sh" mem <command> [options]
```

Do not depend on the current working directory. `--mem-path` is optional; its
default is `%LOCALAPPDATA%\WebUse\Mem` on Windows and
`~/Library/Application Support/WebUse/Mem` on macOS. `WEBUSE_DATA_DIR` overrides
the common data root.

## Claude Code automatic trigger rule

For every concrete Claude Code task involving browser use, websites, Chrome, WebUse, GUI automation, screenshots, mouse/keyboard automation, form filling, web reading, Gmail, Zhihu, or other screen-based web work, Claude Code MUST automatically run this skill at task start and task end.

Do not wait for the user to invoke `/webuse:webuse-mem`. Treat this skill as the default memory layer for WebUse-style tasks.
...
folder bundled beside plugin source code, because plugin files may be read-only
...
At the end of every completed concrete task, Claude Code MUST review the task and decide what belongs in memory. Record only information that is durable, reusable, and likely to help a future Claude Code task. New evidence may require editing, reordering, replacing, or removing old guidance rather than appending another dated note.
...
```powershell
Get-Content -Raw -LiteralPath .\memory-update.md | & "$env:CLAUDE_PLUGIN_ROOT\scripts\webuse.ps1" mem record --task Gmail --file flow.md --title "gmail compose flow" --stdin --json
```

```bash
cat ./memory-update.md | bash "$CLAUDE_PLUGIN_ROOT/scripts/webuse.sh" mem record --task Gmail --file flow.md --title "gmail compose flow" --stdin --json
```
...
```powershell
Get-Content -Raw -LiteralPath .\complete-reconciled-flow.md | & "$env:CLAUDE_PLUGIN_ROOT\scripts\webuse.ps1" mem rewrite --task Gmail --file flow.md --stdin --json
```

```bash
cat ./complete-reconciled-flow.md | bash "$CLAUDE_PLUGIN_ROOT/scripts/webuse.sh" mem rewrite --task Gmail --file flow.md --stdin --json
```
...
```powershell
Get-Content -Raw -LiteralPath .\global-update.md | & "$env:CLAUDE_PLUGIN_ROOT\scripts\webuse.ps1" mem record-global --title "global rule" --stdin --json
```

```bash
cat ./global-update.md | bash "$CLAUDE_PLUGIN_ROOT/scripts/webuse.sh" mem record-global --title "global rule" --stdin --json
```
...
```powershell
Get-Content -Raw -LiteralPath .\complete-reconciled-global.md | & "$env:CLAUDE_PLUGIN_ROOT\scripts\webuse.ps1" mem rewrite-global --stdin --json
```

```bash
cat ./complete-reconciled-global.md | bash "$CLAUDE_PLUGIN_ROOT/scripts/webuse.sh" mem rewrite-global --stdin --json
```
