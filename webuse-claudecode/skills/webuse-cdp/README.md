...
Claude Code Skill for controlling Chrome pages through the Chrome DevTools Protocol (CDP).

Main file: `scripts/webuse-cdp.py`
...
launches and relaunches; see the opening-notice rule in [SKILL.md](SKILL.md).
...
Do not ask the user for a profile path during normal Claude Code use. Reusing this directory keeps login state across days.
...
Use the plugin-level platform installer in `scripts/` to install dependencies.
...
by the command above. Its latest automated run encountered a screenshot timeout
after mouse movement; it has not been marked as passing.
...
Use CDP for browser-internal operations. If a native browser, OS, permission, file picker, or extension popup appears, Claude Code may use `webuse-screenshot`, `webuse-mouse-control`, `webuse-typing`, or `webuse-wait` to inspect and click through that visible popup before returning to CDP.
