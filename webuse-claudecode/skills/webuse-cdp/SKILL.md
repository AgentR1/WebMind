---
name: webuse-cdp
description: Claude Code skill for using a fixed dedicated Chrome DevTools Protocol browser profile to inspect and operate Chrome tabs. Use when Claude Code needs to launch or connect to the persistent local CDP browser, list tabs, select a target tab, navigate URLs, evaluate JavaScript, read DOM state, wait for selectors, click or fill elements by CSS selector, send key events, handle JavaScript dialogs, or capture page screenshots through CDP instead of screen coordinates.
---

# webuse-cdp

Use this skill when Claude Code can operate a Chrome page more reliably through CDP than through visual mouse/keyboard automation.
...
```powershell
# Windows PowerShell
& "$env:CLAUDE_PLUGIN_ROOT\scripts\webuse.ps1" cdp <command> [options]
```

```bash
# macOS
bash "$CLAUDE_PLUGIN_ROOT/scripts/webuse.sh" cdp <command> [options]
```
...
Windows or `scripts/install.sh` on macOS from the plugin root.
...
When using CDP, if a native browser, OS, permission, file picker, extension, download, save, or other visible popup blocks progress, Claude Code may independently use other WebUse skills to handle it:
...
Install all plugin dependencies once with `scripts/install.ps1` on Windows or
...
```text
Windows: %LOCALAPPDATA%\WebUse\chrome-profile
macOS:   ~/Library/Application Support/WebUse/chrome-profile
```
