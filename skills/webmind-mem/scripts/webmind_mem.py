...
def is_link_or_reparse_point(path: Path) -> bool:
    """Reject links/reparse points before path resolution can hide them."""
    try:
        info = path.lstat()
...
def expected_profile(mem_path: Path) -> Dict[str, Any]:
    ...
    # Keep the lexical child path here. Resolving it first would turn a profile
    # symlink/junction into its target and make the subsequent safety check useless.
    profile_path = mem_path / info["profile_name"]
    return {
...
def ensure_profile(mem_path: Path, create: bool) -> Dict[str, Any]:
    ...
    if is_link_or_reparse_point(profile_dir):
        raise ValueError(f"browser profile path must be a real directory, not a link or reparse point: {profile_dir}")
    if not profile_dir.exists():
        if not create:
            missing.append(str(profile_dir))
            return {"created": created, "missing": missing, "profile": expected, "metadata_path": str(metadata_path)}
        profile_dir.mkdir(parents=False, exist_ok=False)
        created.append(str(profile_dir))
    if not profile_dir.is_dir() or is_link_or_reparse_point(profile_dir):
        raise ValueError(f"browser profile path must be a real directory, not a link or reparse point: {profile_dir}")
...
def ensure_structure(mem_path: Path, create: bool) -> Dict[str, Any]:
    ...
    profile_result = ensure_profile(mem_path, create=create)
    created.extend(profile_result["created"])
    missing.extend(profile_result["missing"])
    if create:
        rebuild_content(mem_path)
    return {"created": created, "missing": missing, "profile": profile_result["profile"], "metadata_path": profile_result["metadata_path"]}
...
def initialization_status() -> Dict[str, Any]:
    base: Dict[str, Any] = {
        ...
        "name_rule": "xxx-yyy-mem; xxx=1-8 lowercase English letters; yyy=1-999 without leading zero; port=1000+yyy",
        "uniqueness_rule": "For a new Mem, both xxx and yyy must each be unique among WebMind Mem names used on this computer; do not reuse either existing value. name-info checks format only, not uniqueness.",
        "tutorial_first": True,
        "tutorial_files": ["USER_GUIDE.md", "SAFETY_INSTRUCTIONS.md", "使用教程.md", "安全须知.md"],
...
def command_scan(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    return {
        ...
        "mem_folders": found,
        "uniqueness_notice": "For a new Mem, both xxx and yyy must each be unique among WebMind Mem names used on this computer; this direct-child scan is not a machine-wide uniqueness check.",
    }
...
def command_name_info(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    return {
        ...
        "action": "name-info",
        **info,
        "uniqueness_notice": "For a new Mem, both xxx and yyy must each be unique among WebMind Mem names used on this computer; name-info validates format only.",
    }
...
def command_init(args: argparse.Namespace) -> Dict[str, Any]:
    if not args.accept_risk:
        raise ValueError(
            "initialization requires explicit risk acceptance. Recommend reading the User Guide and Safety Instructions first; "
            "before creating a new Mem, explicitly explain that both xxx and yyy must each be unique among WebMind Mem names used on this computer; "
    ...
    return {
        ...
        "profile_metadata": result["metadata_path"],
        "location_file": str(LOCATION_FILE),
        "reminder": "Remember this Mem folder name and location. For future new Mems, do not reuse this xxx or yyy value; both must remain unique among WebMind Mem names used on this computer. A skill update may require selecting this Mem again. To switch Mem/profile, re-enter initialization before or after a task.",
    }
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = sub.add_parser("init", help="create/attach an external Mem folder and make it the selected Mem/profile; for a new Mem, xxx and yyy must each be unique")
    add_json_flag(p)
    p.add_argument("--mem-path", required=True, help="exact external Mem path named xxx-yyy-mem; for a new Mem, do not reuse an existing xxx or yyy")
