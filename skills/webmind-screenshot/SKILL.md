...
description: Use webmind-screenshot for authorized local macOS Claude Code browser or desktop tasks requiring current desktop geometry and non-sensitive screenshots.
---

# webmind-screenshot - macOS / Claude Code
...
```bash
bash "$WEBMIND_ROOT/scripts/webmind.sh" screenshot <command> [options]
```
...
This edition uses macOS logical desktop points; Retina bitmap pixels are different.
Capture uses mss/Pillow with the system screencapture fallback. Native geometry uses
Quartz without saving a screenshot. Screen Recording and input permissions are separate.
