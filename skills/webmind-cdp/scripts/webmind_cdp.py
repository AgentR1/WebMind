...
def load_browser_configuration() -> Dict[str, Any]:
    ...
    # Check the lexical child path before resolving it. Otherwise a symlink or
    # Windows junction can resolve outside the selected Mem and evade validation.
    profile_dir = mem_path / info["profile_name"]
    metadata_path = profile_dir / PROFILE_METADATA_FILE
    if _is_link_or_reparse_point(profile_dir) or not profile_dir.is_dir():
