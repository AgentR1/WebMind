...
5. Offer any valid existing Mem candidates for the user to select. Otherwise ask for
   a new `xxx-yyy-mem` name: 1-8 lowercase letters, then an integer 1-999 without a
   leading zero, then the mandatory `-mem` suffix. Explicitly explain that both `xxx`
   and `yyy` must each be unique among WebMind Mem folders on the same computer; do
   not reuse either part for another Mem. `name-info` validates the name syntax.
...
`global.md`. Treat both Mem-name components as unique on the same computer: every Mem
should use a distinct `xxx` and a distinct `yyy`. Reusing `yyy` also reuses the same
port. Never kill a conflicting browser to work around a naming or port conflict.
