...
def command_name_info(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    return {"ok": True, "action": "name-info", **info, "uniqueness_notice": "Both xxx and yyy must each be unique among WebMind Mem folders on this computer; do not reuse either part for another Mem."}
...
def command_init(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    return {
        ...
        "profile_metadata": result["metadata_path"],
        "location_file": str(LOCATION_FILE),
        "reminder": "Remember this Mem folder name and location. Both xxx and yyy must each remain unique among WebMind Mem folders on this computer. A skill update may require selecting this Mem again. To switch Mem/profile, re-enter initialization before or after a task.",
    }
...
def command_read(args: argparse.Namespace) -> Optional[Dict[str, Any]]:
    ...
    if args.scope == "global":
        ...
        if args.file:
            requested_file = safe_child(task_dir, safe_file_name(args.file))
            if not requested_file.is_file():
                fail(f"memory file does not exist: {requested_file}", args.json)
