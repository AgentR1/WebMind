...
def parse_mem_name(name: str) -> Dict[str, Any]:
    ...
    if not match:
        raise ValueError(
            ...
            "yyy is 1-999 with no leading zero; -mem is required; for a new Mem, both xxx and yyy must each be unique on this Mac"
        )
...
def command_scan(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    return {"ok": True, "action": "scan", "parent": str(parent), "direct_children_only": True, "mem_folders": found}


def command_name_info(args: argparse.Namespace) -> Dict[str, Any]:
    info = parse_mem_name(args.name)
    return {"ok": True, "action": "name-info", **info, "uniqueness_notice": "For a new Mem, both xxx and yyy must each be unique on this Mac; do not reuse either value."}
...
def command_init(args: argparse.Namespace) -> Dict[str, Any]:
    if not args.accept_risk:
        raise ValueError(
            ...
            "if the user chooses to continue, explain that both xxx and yyy must each be unique for a new Mem on this Mac, then rerun with --accept-risk"
        )
    ...
    return {
        ...
        "profile_metadata": result["metadata_path"],
        "location_file": str(LOCATION_FILE),
        "reminder": "Remember this Mem folder name and location. For every new Mem on this Mac, both xxx and yyy must each be unique; do not reuse either value. A skill update may require selecting this Mem again. To switch Mem/profile, re-enter initialization before or after a task.",
    }
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = sub.add_parser("name-info", help="validate an xxx-yyy-mem name, remind that xxx and yyy must each be unique, and show its derived browser port/profile name")
    add_json_flag(p)
