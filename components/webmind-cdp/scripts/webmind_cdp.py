#!/usr/bin/env python3
"""Small Chrome DevTools Protocol client for webmind-codex tasks."""
...
def find_chrome_executable(explicit_path: Optional[str] = None) -> str:
    ...
    env_path = os.environ.get("WEBMIND_CDP_CHROME")
    if env_path:
        path = Path(env_path).expanduser()
        if path.is_file():
            return str(path)
        raise RuntimeError(f"WEBMIND_CDP_CHROME does not exist: {env_path}")
    ...
    raise RuntimeError("Chrome executable not found. Set --chrome-path or WEBMIND_CDP_CHROME.")
...
def verify_endpoint_profile(args: argparse.Namespace, version: Dict[str, Any]) -> str:
    ...
    try:
        ...
        try:
            ...
            # Existing webmind-codex browsers do not need to be restarted with a new flag.
