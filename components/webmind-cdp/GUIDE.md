...
Run `tabs`, inspect the desired page and specify its real `--target-id` on every command
that operates on an existing tab. The CLI requires the exact target ID and no longer
falls back to URL/title matching or the first available tab. Global flags go before the
subcommand.
