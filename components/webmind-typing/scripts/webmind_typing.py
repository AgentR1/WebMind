#!/usr/bin/env python3
"""Keyboard automation helper for Codex.
...
def english_warning() -> str:
    return (
        "Before this keyboard action, Codex must ensure the active input "
...
def validate_key(key: str) -> str:
    ...
    if normalized not in keys:
        raise ValueError(
            f"Unsupported key '{key}'. Run the webmind-codex `typing list-keys` command "
...
def action_self_check(args: argparse.Namespace) -> Result:
    warnings = [
        ...
        "Before typing, Codex must ensure the active input method is English.",
    ]
    details: dict[str, Any] = {
