#!/usr/bin/env python3
"""Codex-native dispatcher for the retained WebUse components."""
...
from runtime import (ROOT, configure_utf8_stdio, data_dir, venv_python, MINIMUM_PYTHON,
                     require_native_host, is_wsl, macos_permissions, windows_desktop, gui_preflight)

COMPONENTS = {
    'cdp': ROOT / 'components/webuse-cdp/scripts/webuse_cdp.py',
    'screenshot': ROOT / 'components/webuse-screenshot/scripts/webuse_screenshot.py',
    'mouse': ROOT / 'components/webuse-mouse-control/scripts/webuse_mouse_control.py',
    'typing': ROOT / 'components/webuse-typing/scripts/webuse_typing.py',
    'mem': ROOT / 'components/webuse-mem/scripts/webuse_mem.py',
    'wait': ROOT / 'scripts/webuse_wait.py',
}
INPUT_FLAGS = {
...
def prepare_input(component: str, arguments: list[str]) -> tuple[list[str], bytes | None]:
    """File input avoids quoting and legacy PowerShell native-pipeline encoding."""
...
def doctor() -> int:
    native = platform.system() in {'Windows', 'Darwin'} and not is_wsl()
    ...
    if native:
        try:
            permissions = macos_permissions() if platform.system() == 'Darwin' else windows_desktop()
        except Exception as exc:
            permissions = {'check_error': str(exc)}
    chrome = None
    chrome_error = None
    try:
        spec = importlib.util.spec_from_file_location('_webuse_doctor_cdp', COMPONENTS['cdp'])
    ...
    if platform.system() == 'Darwin' and permissions:
        ...
        gui_ready = permissions.get('interactive_default_desktop')
    result = {'ok': runtime_ok, 'runtime_ready': runtime_ok, 'gui_permissions_ready': gui_ready,
              'browser_available': bool(chrome), 'native_host': native, 'wsl': is_wsl(),
              'python': platform.python_version(), 'platform': platform.platform(), 'root': str(ROOT),
              'data_dir': str(data), 'mem_dir': str(data/'Mem'), 'profile_dir': str(data/'chrome-profile'),
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(description='WebUse for local Codex on macOS and Windows. Text commands accept --input-file FILE.')
    ...
    try:
        ...
        env = os.environ.copy()
        env.update({'WEBUSE_CODEX_DATA_DIR': str(data_dir()), 'PYTHONUTF8': '1',
