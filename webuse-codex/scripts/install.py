...
NAME = 'webuse-codex'
VERSION = '2.0.0'
BEGIN = '<!-- webuse-codex:start -->'
END = '<!-- webuse-codex:end -->'
IGNORED = {'.git', '.agents', '.venv', 'venv', '__pycache__', '.pytest_cache', '.DS_Store', INSTALL_MARKER}
...
def install_copy(source: Path, destination: Path, metadata: dict) -> dict:
    ...
    stage = Path(tempfile.mkdtemp(prefix='.webuse-stage-', dir=destination.parent))
    backup = None
    try:
        ...
        if destination.exists():
            ...
            backup_root = destination.parent.parent / '.webuse-backups'
...
def update_agent_rules(path: Path, skill_root: Path) -> dict:
    ...
    block = f'''{BEGIN}
## WebUse for Codex
For an authorized browser or desktop automation task, use `$webuse-codex` and read
...
{END}'''
    if BEGIN in old or END in old:
        if old.count(BEGIN) != 1 or old.count(END) != 1 or old.index(END) < old.index(BEGIN):
            raise ValueError('Malformed WebUse AGENTS markers; refusing to rewrite existing instructions')
    ...
    if new != old:
        ...
        if path.exists():
            backup = path.with_name(path.name + '.webuse-backup-' + uuid.uuid4().hex[:12])
...
def parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description='Install WebUse for Codex on native macOS or Windows. Existing browser profiles and Mem are preserved.')
    ...
    p.add_argument('--add-agent-rules', action='store_true', help='Explicitly add a bounded WebUse block to project AGENTS.md or CODEX_HOME/AGENTS.md')
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        result.update({'ok': True, 'data_dir': str(data), 'dependencies_installed': not args.skip_deps,
                       'next': 'Restart Codex if needed, invoke $webuse-codex, and run doctor --json. macOS GUI permissions and Codex sandbox approvals may still be required.'})
