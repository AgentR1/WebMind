...
def _open_mss(*, with_cursor: bool = False):
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "upgrade WebUse dependencies with the plugin installer"
        )
...
def _read_resolution_with_pillow() -> DesktopInfo:
    if platform.system() in {"Windows", "Darwin"}:
        # Never use a Retina bitmap width as a mouse-coordinate width.
    ...
    return DesktopInfo(backend="pillow_imagegrab", rect=Rect(0, 0, int(width), int(height)), monitors=[])


def _read_resolution_with_macos_screencapture() -> DesktopInfo:
    # Read logical display geometry without creating a privacy-bearing image.
    geometry = native_desktop_geometry()
    return DesktopInfo(backend="quartz_geometry", rect=Rect(**geometry["rect"]), monitors=geometry["monitors"])
...
def read_resolution() -> DesktopInfo:
    ...
    raise RuntimeError(
        ...
        "the WebUse project installer, make sure a graphical session is active, "
...
def _capture_rect_with_mss(rect: Rect, output_path: Path, *, with_cursor: bool = True) -> Tuple[str, bool]:
    ...
    request_native_cursor = with_cursor and platform.system() == "Linux"
...
def _capture_rect_with_macos_screencapture(rect: Rect, output_path: Path) -> str:
    # -R expects x,y,width,height.
    region = f"{rect.left},{rect.top},{rect.width},{rect.height}"
    subprocess.run(["screencapture", "-x", "-R", region, str(output_path)], check=True)
...
def capture_rect(rect: Rect, output_path: Path, *, with_cursor: bool = True) -> Tuple[str, bool]:
    ...
    if platform.system() == "Darwin":
        try:
            return _capture_rect_with_macos_screencapture(rect, output_path), False
        except Exception as exc:
            errors.append(f"macos_screencapture: {exc}")
...
def _overlay_cursor_marker(
    ...
    result: Dict[str, Any] = {
        ...
        "position": {"x": x, "y": y, "coordinate_space": "macos-logical-points" if platform.system() == "Darwin" else "desktop-absolute-pixels"},
    }
...
def _base_result(task: str, desktop_info: DesktopInfo) -> Dict[str, Any]:
    return {
        ...
        "desktop_coordinate_space": "macos-logical-points" if platform.system() == "Darwin" else "desktop-absolute-pixels",
...
def _self_check(as_json: bool) -> int:
    ...
    try:
        ...
        pass
    if platform.system() == "Darwin":
        result["macos_screencapture_available"] = bool(
            subprocess.run(["which", "screencapture"], capture_output=True).stdout.strip()
        )

    result["ok"] = bool(
        result["mss_available"]
        or result["pillow_available"]
        or result["macos_screencapture_available"]
    )
    if not result["ok"]:
