...
Use this skill when Codex needs to control the local mouse pointer during desktop or browser automation.
...
```powershell
& "$WebUseRoot\scripts\webuse.ps1" mouse <command> [options]
```

```bash
bash "$WEBUSE_ROOT/scripts/webuse.sh" mouse <command> [options]
```

Do not depend on the current working directory. If dependencies are missing,
run the project's platform-specific installer first.
...
The program uses `pyautogui` and `mss`. Install the project dependencies with
the platform-specific installer in the project's root `scripts` directory.
...
Codex MUST confirm the cursor position with a screenshot before every click-style action and before every mouse wheel scroll action. This rule applies to:
...
Use this when Codex has selected a target point such as `x=500, y=300`.
...
Use small amounts first, such as `3` to `8`, then repeat after another screenshot if more scrolling is needed. Codex must screenshot-confirm the cursor position immediately before every scroll action.
