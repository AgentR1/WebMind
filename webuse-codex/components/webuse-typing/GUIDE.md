...
Use this skill when Codex needs to simulate real keyboard input in the active desktop session.
...
```powershell
& "$WebUseRoot\scripts\webuse.ps1" typing <command> [options]
```

```bash
bash "$WEBUSE_ROOT/scripts/webuse.sh" typing <command> [options]
```
...
The program uses `pyautogui`. Install the project dependencies with the
platform-specific installer in the project's root `scripts` directory.
...
```powershell
"hello world" | & "$WebUseRoot\scripts\webuse.ps1" typing type --stdin --interval 0.02
```

```bash
printf %s "hello world" | bash "$WEBUSE_ROOT/scripts/webuse.sh" typing type --stdin --interval 0.02
```
...
Use `command` on macOS where the user expects the Command key. Use `ctrl` for Windows shortcuts.
